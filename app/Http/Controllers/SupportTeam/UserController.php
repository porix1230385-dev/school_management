...
class UserController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        $d['class_sections'] = $this->my_class->getAllClassSections();
    ...
    public function store(UserRequest $req)
    {
        ...
        $data['lastname'] = ucwords($req->lastname);
    ...
    public function update(UserRequest $req, $id)
    {
        ...
        try {
            if ($user_type == 'teacher') {
                ...
                $my_class_sections = $req->get('class_section_id');
                for ($i = 0; $i < count($my_class_sections); $i++) {
                    $my_data = [
                        'teacher_id' => TeacherRecord::where('user_id', $id)->first()->id,
                        'class_section_id' => $my_class_sections[$i],
                        'year' => $this->year
                    ];
                    // dd($my_data);
                    Teach::create($my_data);
                }
                // foreach($my_class_sections as $cs){
                //     // $cs = (int)$cs;

                // }
            }
        } catch (Exception $exc) {
            return response()->json(['ok' => false, 'msg' => $exc->getMessage()]);
        }
